...
\input{crypt4gh_v2.ver}
\title{GA4GH File Encryption Standard, version 2}
\date{\headdate}
\maketitle
\begin{quote}\small
...
\node (header) [boxes=7,below=3.5 of file.two split south] {
\nodepart{one}Magic \\ ``crypt4gh''
\nodepart{two}Version
\nodepart{three}Header Packet Encryption Method
\nodepart{four}Writer's Public Key $K_{pw}$
\nodepart{five}Packet Count
\nodepart{six}Header Packet
\nodepart{seven}Header Packet...
};
...
\node (header packet) [boxes=4,below=of header.six south] {
\nodepart{one}Packet Length
\nodepart{two}Nonce
\nodepart[text width=10em]{three}Encrypted Packet Data
\nodepart{four}MAC
};
\draw (header.five split south) to (header packet.north west);
\draw (header.six split south) to (header packet.north east);
...
\draw (header packet.two split south) to (data encryption packet.north west);
\draw (header packet.three split south) to (data encryption packet.north east);
...
\item The method used to encrypt header packets.
\item The writer's public key  ($K_{pw}$) used to encrypt the header packets.
This is needed (along with the reader's secret key $K_{sr}$)
to calculate the shared key used to encrypt the header packets.
\item The header packet count.
\item One or more header packets containing:
\begin{itemize}
\item The packet length in bytes.
\item A random ``nonce'', required for decryption.
...
\hline
\datatype{byte} & & unsigned & 1 \\
\datatype{le\_int32} & little-endian & signed & 4 \\
\datatype{le\_uint32} & little-endian & unsigned & 4 \\
\datatype{le\_int64} & little-endian & signed & 8 \\
\datatype{le\_uint64} & little-endian & unsigned & 8 \\
\datatype{le\_uint96} & little-endian & unsigned & 12 \\
\end{tabular}
\end{center}
...
\begin{verbatim}
0x63 0x72 0x79 0x70 0x74 0x34 0x67 0x68 0x02 0x00 0x00 0x00
============= magic_number============= ===== version =====
\end{verbatim}

\kw{packet\_encryption} is the encryption method used for all header packets.

\kw{writer\_public\_key} ($K_{pw}$) is the public key needed to decrypt
all of the encrypted payload in the header packets.
...
\kw{nonce} is a parameter needed to decrypt the \kw{encrypted\_payload} in the
packet.
...
enum Data_encryption_method<le_uint32> {
  ...
  chacha20_ietf_poly1305_with_AD = 1;
};
    ...
    case data_encryption_parameters:
      ...
      select (data_encryption_method) {
        case chacha20_ietf_poly1305:
          byte  data_key[32];
        case chacha20_ietf_poly1305_with_AD:
          byte      data_key[32];
          le_uint64 initial_sequence_number;
      };
...
The \kw{initial\_sequence\_number} is an eight byte unsigned integer (\datatype{le\_uint64}).
It is used by the \kw{chacha20\_ietf\_poly1305\_with\_AD} method to set the
base value for a counter that is used to validate the order of encrypted blocks
in the file.
See section~\ref{data:AD_encrypting_mode} for details of how this is applied.
...
If the data encryption methods are mixed, the file MUST be rejected.
...
\subsubsection{data\_location packet}

This packet contains a URL linking to a location where the data blocks to be
decrypted can be found.
There can be at most one data\_location packet in the header.
If present, the header will not be followed by any data blocks,
instead the reader should use the URL to access them.
...
To decrypt the header packet, the reader obtains the writer's public key $K_{pw}$ from the header
and the nonce from the beginning of the packet.
...
For the \kw{chacha20\_ietf\_poly1305} encryption method,
the AAD is zero bytes long.

For the \kw{chacha20\_ietf\_poly1305\_with\_AD} encryption method,
the AAD is an eight byte little-endian number as described in the next section.

\subsubsection{chacha20\_ietf\_poly1305\_with\_AD Encryption}\label{data:AD_encrypting_mode}

While the \kw{chacha20\_ietf\_poly1305} MAC ensures individual file segments are unmodified,
it is not able to detect changes where entire blocks have been reordered or removed.
To find such changes, the \kw{chacha20\_ietf\_poly1305\_with\_AD} method makes
use of additional authenticated data to enforce ordering of the file segments.
It also changes how the file is terminated so that processes reading to the end
can tell if it has been truncated.

Each data segment is given an index number, in order,
counting from $0$ at the start of the file.
The index number is then added modulo $2^{64}$ to the \kw{initial\_sequence\_number}
in the \kw{data\_encryption\_parameters} header packet.
The resulting \datatype{le\_uint64} value is used as the AAD in the Poly1305
message described in section 2.8 of \cite{RFC8439}.
This makes the MAC depend on the location of the data segment in the file.
Attempting to reorder segments or delete them will cause decoding to fail,
either on the reordered segment,
or the following ones in the case of a deletion.

To detect file truncation,
where all blocks to the end have been deleted,
readers can inspect the size of the last segment.
As the format only permits the last segment to hold fewer than 65536 plain-text bytes,
successfully decrypting a short segment at the end of the file assures that the correct end-of-file has been reached.
To allow for files where the plain-text is an exact multiple of 65536 bytes long,
writers MUST append an extra segment with zero-length plain-text if (and only if)
the last segment was 65536 bytes long.
Where present,
the AAD and MAC for this extra segment will be calculated in the normal way.
When written out,
this extra segment will consist of the 12 byte nonce value followed immediately by the 16 byte MAC.

\section{Decryption}

\subsection{chacha20\_ietf\_poly1305 and chacha20\_ietf\_poly1305\_with\_AD Decryption}
...
first be found.
For the both methods, when no edit list is in use, this can be done using the formula:

\begin{verbatim}
seg_start = header_len + floor(P/65536) * 65564
\end{verbatim}

For \kw{chacha20\_ietf\_poly1305\_with\_AD} encryption,
the additional data is also calculated using the formula:

\begin{verbatim}
AD = (initial_sequence_number + floor(P/65536)) modulo 18446744073709551616
\end{verbatim}
...
An authentication tag is calculated over the cipher-text from that segment,
along with the additional data (for \kw{chacha20\_ietf\_poly1305\_with\_AD} only),
and bit-wise compared to the MAC.
...
The chacha20\_ietf\_poly1305 encryption method does not protect against
block-level changes such as removing or reordering blocks.
By adding extra checks on block ordering,
the chacha20\_ietf\_poly1305\_with\_AD method will detect these changes,
ensuring full-file integrity.
To prevent downgrade attacks,
implementations MUST reject files that mix the two encryption methods,
and SHOULD default to encryption using the chacha20\_ietf\_poly1305\_with\_AD method.
...
The keys MUST be generated using a cryptographically-secure random number generator.
...
For a 96-bit nonce, it may be acceptable to generate each one using a cryptographically-secure random number
generator.
...
Using ChaCha20-IETF-Poly1305 authenticates the content of the header packets and each segment of the encrypted
cipher-text.
As all header packets use the same writer's public key,
it is not possible to add any packets without holding the corresponding secret key.
It is possible to re-order or remove header packets,
and to re-write the entire header using a new key.
To usefully alter the header would require access to either the writer's or a reader's secret key,
and so also to the unencrypted file content.

The chacha20\_ietf\_poly1305 data encryption method does not protect against the addition,
removal or rearrangement of data segments.
...
\section{Changes from previous crypt4gh specifications}

\subsection{crypt4gh version 2}

\begin{itemize}
\item The version number in the header has changed from 1 to 2.

\item The writer's public key has been moved from the header packets to the common header at the start of the file.
This closes a loop-hole that allowed extra header packets to be added to an existing header,
by generating a new writer's key and using it along with the known reader's public key to encrypt the packets.
As all header packets must now share the same writer's public key,
only holders of the corresponding secret key can generate new header packets.

\item The \kw{chacha20\_ietf\_poly1305\_with\_AD} encryption method has been added.
This gives whole-file authentication by ensuring that blocks cannot be reordered or removed,
and that the file has not been truncated.
As with the original \kw{chacha20\_ietf\_poly1305} method,
the individual blocks are authenticated by the block-level MAC.
